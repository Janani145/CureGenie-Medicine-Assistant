...
# -----------------------------
# Helper: symptoms dictionary + disease map (as given)
# -----------------------------
symptoms_dict = {
...
# -----------------------------
# Utility: normalize a symptom string to best match keys
# (tries raw lower, replace spaces with underscores, and the inverse)
# -----------------------------
def normalize_symptom(s: str) -> str:
    t = s.strip().lower()
    if t in symptoms_dict:
        return t
    t_us = t.replace(" ", "_")
    if t_us in symptoms_dict:
        return t_us
    t_sp = t.replace("_", " ")
    if t_sp in symptoms_dict:
        return t_sp
    return t  # fallback (will be flagged invalid later)

# -----------------------------
# Helper: fetch description, precautions, meds, diets, workouts for a disease
# Returns flat Python lists/strings for template
# -----------------------------
def helper(dis: str):
    # Description (single string)
    desc_series = description[description['Disease'] == dis]['Description']
    desc = " ".join([w for w in desc_series]) if not desc_series.empty else ""

    # Precautions (four columns -> take row values, flatten)
    ...
    if not pre_df.empty:
        ...
        pre_list = [p for p in first_row if isinstance(p, str) and p.strip()]

    # Medications (one-per-row)
    med_series = medications[medications['Disease'] == dis]['Medication']
    med_list = [m for m in med_series.values if isinstance(m, str) and m.strip()]

    # Diets (one-per-row)
    diet_series = diets[diets['Disease'] == dis]['Diet']
    diet_list = [d for d in diet_series.values if isinstance(d, str) and d.strip()]

    # Workouts (one-per-row)
    wrk_series = workout[workout['disease'] == dis]['workout']
    wrk_list = [w for w in wrk_series.values if isinstance(w, str) and w.strip()]

    return desc, pre_list, med_list, diet_list, wrk_list
...
def get_predicted_value(patient_symptoms):
    ...
    pred_idx = int(svc.predict([input_vector])[0])
    return diseases_list.get(pred_idx, "Unknown")
...
def home():
    if request.method == 'POST':
        ...
        # Empty input
        if not symptoms_raw:
            message = "⚠️ Please type your symptoms before predicting."
            return render_template('index.html', message=message)

        # Split, clean, normalize each symptom token
        tokens = [t for t in [tok.strip() for tok in symptoms_raw.split(",")] if t]
        normalized = [normalize_symptom(t) for t in tokens]

        # Validate against known symptoms
        invalid = [t for t in normalized if t not in symptoms_dict]
        if invalid:
            # Show exactly which ones were invalid
            message = f"⚠️ Invalid symptom(s): {', '.join(invalid)}. Please type correct symptom(s)."
            return render_template('index.html', message=message)

        # Predict
        try:
            predicted_disease = get_predicted_value(normalized)
        except Exception as e:
            # Fallback error message if model fails
            message = "⚠️ Something went wrong while predicting. Please check your input and try again."
            return render_template('index.html', message=message)
